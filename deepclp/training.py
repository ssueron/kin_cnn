...
def train_predictor(
    ...
    if model.task_type == "classification":
        ...
        metrics = ["accuracy"]
    elif model.task_type in ["bounded_regression", "regression"]:
        loss = "mean_squared_error"
        metrics = [keras.metrics.RootMeanSquaredError()]
    else:
        raise ValueError(f"Unknown task_type: {model.task_type}")
    ...
    if balance_loss:
        if model.task_type != "classification":
            raise ValueError("Loss balancing is only supported for classification tasks.")
...
def evaluate_predictor(
    ...
    # For both bounded_regression and regression tasks
    return metrics.evaluate_predictions(
